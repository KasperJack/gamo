...
// Get single curative intervention
export async function GET(request, { params }) {
  try {
    ...
    const result = await sql`
      ...
      WHERE mi.id = ${id} AND mi.intervention_type = 'curative'
    `;

    if (result.length === 0) {
      return Response.json({ error: 'Curative intervention not found' }, { status: 404 });
    }

    return Response.json({ intervention: result[0] });
  } catch (error) {
    console.error('Error fetching curative intervention:', error);
    return Response.json({ error: 'Failed to fetch curative intervention' }, { status: 500 });
  }
}

// Update curative intervention
export async function PUT(request, { params }) {
  try {
    ...
    const {
      equipment_id, title, description, priority, status, assigned_technician,
    ...
    const result = await sql`
      ...
      WHERE id = ${id} AND intervention_type = 'curative'
      RETURNING *
    `;
    ...
    console.error('Error updating curative intervention:', error);
    return Response.json({ error: 'Failed to update curative intervention' }, { status: 500 });
  }
}

// Delete curative intervention
export async function DELETE(request, { params }) {
  try {
    ...
    return Response.json({ message: 'Curative intervention deleted successfully' });
  } catch (error) {
    console.error('Error deleting curative intervention:', error);
    return Response.json({ error: 'Failed to delete curative intervention' }, { status: 500 });
  }
}